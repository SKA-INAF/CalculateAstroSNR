...
from utils.data import get_output_path, load_fits_image, read_samples, save_to_json     # for loading fits images with optional normalisation
...
def main(args):
    ...
    images_to_snr_3sigma : Dict = {}
    images_to_snr_mad : Dict = {}
    ...
    for sample in tqdm(samples, desc='Going through images and their object masks, and calculating MAD'):
        ...
        snr_3sigma = peak_flux / three_sigma_clip
        snr_mad = peak_flux / mad

        # Add entry for each image path
        img_path = get_output_path(sample)
        images_to_snr_3sigma[img_path] = snr_3sigma
        images_to_snr_mad[img_path] = snr_mad

    # Save our dictionaries to JSON
    save_to_json(images_to_snr_3sigma, 'images_to_snr_3sigma.json')
    save_to_json(images_to_snr_mad, 'images_to_snr_mad.json')
